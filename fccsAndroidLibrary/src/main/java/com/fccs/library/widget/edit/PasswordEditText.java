package com.fccs.library.widget.edit;
...
public class PasswordEditText extends MaterialEditText {
	...
	// 资源
	private final int INVISIBLE = R.drawable.lib_ic_close;
	private final int VISIBLE = R.drawable.lib_ic_open;
	// 按钮宽度dp
	private int mWidth;
	// 按钮的bitmap
	...
	protected void drawClear(int translationX, Canvas canvas) {
		...
		int right = (int) (getWidth() + getScrollX() - Interval - mWidth - Interval
				- getmWidth_clear() * (1f - scale) / 2f);
		int left = (int) (getWidth() + getScrollX() - Interval - mWidth - Interval
				- getmWidth_clear() * (scale + (1f - scale) / 2f));
